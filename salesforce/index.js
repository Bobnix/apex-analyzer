...
exports.login = function(username, password, callback){
  ...
  conn.login(username, password, function(err, userInfo) {
    ...
    callback(conn.accessToken, conn.instanceUrl);

  });
}

exports.getClassData = function(username, password, callback){
  var conn = new sf.Connection({
    // you can change loginUrl to connect to sandbox or prerelease env.
    loginUrl : 'https://login.salesforce.com',
    maxRequest : 1000
  ...
  var createApexClassMember = function(tempClassMembers, containerId){
    ...
    //conn.bulk.load("ApexClassMember", "insert", tempClassMembers, function(err, rets) {
      if (err) { return console.error(err); }
...
var getSavedConnection = function(instanceUrl, accessToken){
